...
fn cast_heal(
    ...
    if let Some(fighter) = objects[PLAYER].fighter {
        ...
        game.messages
            .add("Your wounds start to fell better!", LIGHT_VIOLET);
...
fn menu<T: AsRef<str>>(header: &str, options: &[T], width: i32, root: &mut Root) -> Option<usize> {
    assert!(
        options.len() <= 26,
        "Cannot have a menu with more that 26 options."
    );
    ...
    // convert the ascii code to index. if correspond to an option, return it
